...
public class ProductController {
    ...
    public ResponseEntity<ProductDTO> getOne(@PathVariable("pno") Long pno){
        log.info("데이터 하나 조회 product controller pno=> {}", pno);
        ProductDTO data = service.get(pno);
        log.info("dto:{}",data);
    ...
    @GetMapping("/list") //Page당 전체 목록 조회
    ...
    public Map<String, String> modify(@PathVariable("pno") Long pno, ProductDTO dto){
        ...
        //수정 작업
        service.modify(dto);
        if(oldFileNames != null && oldFileNames.size() > 0){//카리나.jpg 지젤.jpg / 지젤삭제 카리나.jpg만 넘어옴
            //지워야하는 파일목록 찾기
            //예전 파일들 중에서 지워져야 하는 파일 이름들
            List<String> removeFiles = oldFileNames
                    .stream().filter(i -> !uploadedFileNames.contains(i)).toList();//검색되지 않으면 -1이 반환된
            //실제 파일 삭제
